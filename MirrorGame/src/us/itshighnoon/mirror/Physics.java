...
public class Physics {
	...
	public static float nearestLine(Vector2f p, Wall[] colliders) {
	...
	public static float distToLine(Vector2f p, Wall line) {
		...
		float dx1 = p.x - line.getA().x;
		float dy1 = p.y - line.getA().y;
		float dx2 = p.x - line.getB().x;
		float dy2 = p.y - line.getB().y;
		
		float aDist = (float)Math.sqrt(dx1 * dx1 + dy1 * dy1);
		float bDist = (float)Math.sqrt(dx2 * dx2 + dy2 * dy2);
		if (aDist < dist) dist = aDist;
